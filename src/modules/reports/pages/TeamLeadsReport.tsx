...
export default function TeamLeadsReport() {
  ...
  const totalIncentives = mockData.reduce((sum, row) => sum + row.incentivesToDate, 0);
  const totalApproved = mockData.reduce((sum, row) => sum + row.siteBookingsApproved, 0);
  const totalKms = mockData.reduce((sum, row) => sum + row.kms, 0);

  const metrics = [
    { label: "Total Leads Closed", value: totalClosed, format: "number" as const, trend: { value: 18.5, isPositive: true } },
    { label: "Total Incentives", value: totalIncentives, format: "currency" as const, trend: { value: 22.3, isPositive: true } },
    { label: "Bookings Approved", value: totalApproved, format: "number" as const, trend: { value: 12.8, isPositive: true } },
