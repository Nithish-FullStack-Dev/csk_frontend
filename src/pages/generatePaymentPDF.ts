...
export const generatePaymentPdf = async (paymentData: PaymentData): Promise<Uint8Array> => {
  ...
  const drawField = (label: string, value: string, x: number, y: number) => {
    ...
    const cleanLabel = label.replace(/[^\x00-\x7F]/g, "");
  ...
  // Left column
  drawField('Invoice Number', paymentData.invoice.invoiceNumber, leftColumnX, yPosition);
  yPosition -= rowHeight;
  ...
  page.drawText(
    `Issued on: ${new Date(paymentData.invoice.issueDate).toLocaleDateString()}`,
    {
      x: 50,
      y: yPosition,
      size: 10,
  ...
  if (paymentData.invoice.notes) {
    ...
    if (cleanNotes.length > wordsPerLine) {
      const lines = cleanNotes.match(new RegExp(`.{1,${wordsPerLine}}`, 'g')) || [cleanNotes];
...
export const downloadPaymentPdf = async (paymentData: PaymentData, filename?: string) => {
  try {
